...
- 上报协议兼容原 Windows 上位机：`POST /PCM` 表单
...
- 修复 CPU 采集字段边界判断，并在缺少 `MemAvailable` 时避免误报 100% 内存占用。
- 守护进程校验 HTTP 200 与设备回执令牌 `0637`，避免把失败上报记录为成功。
- LuCI 上报设置保留固定配置段并开放全部参数手动填写，增加「启用上报」控制开关说明。
- 补充 OpenWrt 最新 LuCI/rpcd 权限与安装说明，并加入 MIT 许可证。
